...
import cnnFeature.dimConfig as dimConfig
...
class BaseEmbedder(torch.nn.Module):
    def __init__(self,
        in_dim: int = dimConfig.dim, #in_dim changes the dimension of inputs. Other dimensions avalible here
        embed_dim: int = 768,
        #embed_dim: int = 769,
        num_hidden_layers: int = 1,
        dropout: float = 0.1,
        ...
        **kwargs
        ...
        self.wt_one = torch.nn.Embedding(
            num_embeddings=1+1,  # we add one "dummy" t_r_embedding for cls/seq/msk_emebds
            embedding_dim=1,
        )
    ...
    # def embed_graph_inputs(
    #     self,
    #     graph_inputs: torch.tensor
    #     ) -> torch.tensor:
    #     return self.embed_model(graph_inputs)
    ...
    def embed_1(
        ...
        # one.to("cuda:0")
        #print(one)
        return self.wt_one(one)
    
    def forward(
        self,
        batch: Dict[str, torch.tensor]
        ) -> torch.tensor:
        # print("embeder graph input")
        # print(batch.keys())
        ...
        # print(one.device)
        # print(one)
        one_embed = self.embed_1(one)
        # print('start')
        # t_r_embeds=t_r_embeds.to("cpu")
        # inputs_embeds=inputs_embeds.to("cpu")
        # print(t_r_embeds)
        # print(batch['t_rs'])
        # print(t_r_embeds)
        # print(one_embed)
        #graph_inputs_embeds=self.embed_graph_inputs(graph_inputs=batch['graph_inputs'])
        return inputs_embeds + t_r_embeds# + graph_inputs_embeds + one_embed
